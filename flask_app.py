# flask_app.py - Full MongoDB Version
...
if USE_MONGODB:
    ...
    print("❌ MONGO_URI not set! Please set MONGO_URI environment variable.")
    print("⚠️ Falling back to SQLite (data will be lost on restart)")
    import sqlite3
    # SQLite fallback functions will be defined below
...
# ============================================================
# SQLITE FALLBACK FUNCTIONS (if MongoDB not available)
# ============================================================
...
if not USE_MONGODB:
    ...
    def init_db_sqlite():
        ...
        if not lock:
            conn.execute('''INSERT INTO system_attendance_lock (is_locked, auto_unlock_time) VALUES (0, '06:00')''')
            print("✅ Inserted default system lock record")
    ...
    def create_user_sqlite(username, password, full_name, email=None, phone=None, role='user'):
        conn = get_db_connection()
        try:
    ...
    def get_system_lock_status_sqlite():
        try:
            conn = get_db_connection()
    ...
    def update_system_lock_sqlite(is_locked, lock_start_time=None, lock_end_time=None, auto_unlock_time=None, locked_by=None):
        try:
            conn = get_db_connection()
            current = conn.execute("SELECT * FROM system_attendance_lock ORDER BY id DESC LIMIT 1").fetchone()
            if current:
                conn.execute('''UPDATE system_attendance_lock SET is_locked = ?, lock_start_time = COALESCE(?, lock_start_time),
                    lock_end_time = COALESCE(?, lock_end_time), auto_unlock_time = COALESCE(?, auto_unlock_time),
                    locked_by = COALESCE(?, locked_by), updated_at = CURRENT_TIMESTAMP WHERE id = ?''',
                    (is_locked, lock_start_time, lock_end_time, auto_unlock_time, locked_by, current['id']))
            else:
                conn.execute('''INSERT INTO system_attendance_lock (is_locked, lock_start_time, lock_end_time, auto_unlock_time, locked_by)
                    VALUES (?, ?, ?, ?, ?)''', (is_locked, lock_start_time, lock_end_time, auto_unlock_time, locked_by))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    # Map SQLite functions
    init_db = init_db_sqlite
    get_user_by_id = get_user_by_id_sqlite
    get_user_by_username = get_user_by_username_sqlite
    create_user = create_user_sqlite
    get_system_lock_status = get_system_lock_status_sqlite
    update_system_lock = update_system_lock_sqlite
        ...
        for user in all_users:
            ...
            if user_lock.get('is_locked', 0) == 1:
                ...
                if auto_unlock and current_time >= auto_unlock:
                    ...
                    print(f"✅ User {user_id} auto-unlocked via API at {current_time}")
        if unlocked_items:
            increment_data_version()
...
def login():
    if request.method == 'POST':
        ...
        if user and user['password'] == password:
            ...
            session['user_id'] = user.get('id', str(user.get('_id')))
...
# ============================================================
# HTML TEMPLATES - DASHBOARD_HTML, REGISTER_HTML, CHANGE_PASSWORD_HTML, USER_MANAGEMENT_HTML, REPORT_HTML
# ============================================================

# [NOTE: Due to length, I'm including the DASHBOARD_HTML here.
# For the other HTML templates (REGISTER_HTML, CHANGE_PASSWORD_HTML, USER_MANAGEMENT_HTML, REPORT_HTML),
# please use the ones from your original flask_app.py file as they are unchanged.]
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: linear-gradient(135deg, #1a73e8, #0d47a1); color: white; padding: 12px 20px; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(26,115,232,0.3); margin-bottom: 15px; flex-wrap: wrap; gap: 10px; }
        .header-left h2 { font-size: 18px; font-weight: 600; white-space: nowrap; }
        .header-right { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        .header-right .nav-btn { color: white; text-decoration: none; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 500; transition: all 0.3s; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.15); white-space: nowrap; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; cursor: pointer; }
        .header-right .nav-btn:hover { background: rgba(255,255,255,0.25); transform: translateY(-2px); }
        .header-right .nav-btn.location-btn { background: rgba(255,193,7,0.3); border-color: #ffc107; }
        .header-right .nav-btn.location-btn:hover { background: rgba(255,193,7,0.5); }
        .header-right .user-name-btn { color: white; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 500; background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.25); white-space: nowrap; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; cursor: default; display: flex; align-items: center; gap: 6px; }
        .header-right .logout-link { color: white; text-decoration: none; background: rgba(255,255,255,0.2); padding: 6px 16px; border-radius: 20px; font-size: 13px; transition: all 0.3s; font-weight: 500; border: 1px solid rgba(255,255,255,0.15); font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; }
        .header-right .logout-link:hover { background: rgba(255,255,255,0.35); transform: scale(1.05); }
        .stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px; }
        .stat-card { background: white; padding: 18px 15px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); text-align: center; transition: all 0.3s; }
        ...
        .action-buttons .btn { padding: 14px 10px; border: none; border-radius: 12px; cursor: pointer; font-size: 16px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; transition: all 0.3s; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; font-weight: 500; min-height: 65px; color: white; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
        ...
        .action-buttons .btn.disabled { opacity: 0.5; cursor: not-allowed; transform: none !important; }
        ...
        .table-container .table-title { font-size: 18px; font-weight: 600; color: #333; margin-bottom: 15px; display: flex; align-items: center; gap: 10px; }
        .table-container .table-title .badge-count { background: #1a73e8; color: white; padding: 2px 12px; border-radius: 20px; font-size: 14px; }
        table { width: 100%; border-collapse: collapse; }
        table thead th { background: #f8f9fa; color: #555; padding: 12px 15px; text-align: left; font-size: 13px; font-weight: 600; border-bottom: 2px solid #e8ecf1; }
        table tbody td { padding: 12px 15px; border-bottom: 1px solid #f0f2f5; color: #333; font-size: 14px; }
        ...
        .shift-badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; }
        ...
        .modal-content { background: white; padding: 30px; border-radius: 16px; max-width: 500px; width: 90%; box-shadow: 0 10px 40px rgba(0,0,0,0.3); max-height: 90vh; overflow-y: auto; }
        .modal-content h3 { color: #333; margin-bottom: 10px; }
        .modal-content p { color: #666; font-size: 14px; margin-bottom: 15px; }
        .modal-content input, .modal-content textarea, .modal-content select { width: 100%; padding: 10px 14px; border: 2px solid #e8ecf1; border-radius: 10px; font-size: 14px; margin-bottom: 10px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; }
        .modal-content input:focus, .modal-content textarea:focus, .modal-content select:focus { outline: none; border-color: #1a73e8; }
        .modal-content .btn-group { display: flex; gap: 10px; flex-wrap: wrap; }
        .modal-content .btn-group button { flex: 1; padding: 12px; border: none; border-radius: 10px; cursor: pointer; font-size: 15px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; font-weight: 500; min-width: 80px; }
        ...
        .btn-get-location { background: #34a853; color: white; padding: 10px; border: none; border-radius: 10px; cursor: pointer; font-size: 14px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; width: 100%; margin-bottom: 10px; }
        .btn-get-location:hover { background: #1e7e34; }
        .location-info { background: #f8f9fa; padding: 10px; border-radius: 8px; font-size: 13px; color: #555; margin-bottom: 10px; }
        .request-item { background: #f8f9fa; padding: 12px 15px; border-radius: 10px; margin-bottom: 10px; border-left: 4px solid #ffc107; }
        ...
        .request-item .request-actions button { padding: 5px 15px; border: none; border-radius: 8px; cursor: pointer; font-size: 13px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; }
        .request-item .request-actions .approve-btn { background: #34a853; color: white; }
        .request-item .request-actions .approve-btn:hover { background: #1e7e34; }
        .request-item .request-actions .reject-btn { background: #dc3545; color: white; }
        .request-item .request-actions .reject-btn:hover { background: #b02a37; }
        .status-message { padding: 10px 15px; border-radius: 8px; margin-bottom: 10px; font-size: 14px; display: none; }
        ...
        .status-message.warning { background: #fff3cd; color: #856404; border: 1px solid #ffc107; display: block; }
        .status-message.danger { background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; display: block; }
        .status-message.locked { background: #f8d7da; color: #721c24; border: 2px solid #dc3545; display: block; font-weight: 600; }
        .install-btn { background: #34a853; color: white; border: none; border-radius: 20px; padding: 6px 16px; cursor: pointer; font-size: 13px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; font-weight: 500; display: none; animation: pulse-green 2s infinite; }
        @keyframes pulse-green { 0% { box-shadow: 0 0 0 0 rgba(52,168,83,0.4); } 70% { box-shadow: 0 0 0 10px rgba(52,168,83,0); } 100% { box-shadow: 0 0 0 0 rgba(52,168,83,0); } }
        ...
        .summary-card { background: white; border-radius: 12px; padding: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
        .summary-card .card-title { font-size: 15px; font-weight: 600; color: #555; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 2px solid #f0f2f5; }
        .summary-item { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #f5f6f8; font-size: 14px; }
        ...
        .deadline-badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; background: #ff9800; color: white; }
        .deadline-badge.active { background: #4caf50; }
        .deadline-badge.inactive { background: #9e9e9e; }
        .user-lock-badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; background: #dc3545; color: white; }
        .user-lock-badge.unlocked { background: #4caf50; }
        .lock-banner { background: linear-gradient(135deg, #dc3545, #b02a37); color: white; padding: 12px 20px; border-radius: 12px; display: none; align-items: center; justify-content: space-between; margin-bottom: 15px; flex-wrap: wrap; gap: 10px; }
        ...
        .lock-banner .lock-btn { background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); color: white; padding: 6px 16px; border-radius: 20px; cursor: pointer; font-size: 13px; font-family: 'Khmer OS', 'Khmer OS Muol', 'Arial', sans-serif; transition: all 0.3s; }
        ...
        .user-lock-banner { background: linear-gradient(135deg, #ff9800, #e65100); color: white; padding: 10px 16px; border-radius: 12px; display: none; align-items: center; justify-content: space-between; margin-bottom: 15px; flex-wrap: wrap; gap: 8px; }
        ...
        .user-lock-banner .lock-time { font-size: 12px; opacity: 0.8; }
        @media (max-width: 900px) { .header { flex-direction: column; align-items: center; gap: 12px; padding: 15px; } .header-left h2 { font-size: 16px; text-align: center; } .header-right { justify-content: center; } .header-right .nav-btn { font-size: 12px; padding: 5px 12px; } .header-right .user-name-btn { font-size: 12px; padding: 5px 12px; } .header-right .logout-link { font-size: 12px; padding: 5px 12px; } }
        @media (max-width: 768px) { .stats { grid-template-columns: repeat(2, 1fr); gap: 10px; } .stat-card .number { font-size: 24px; } .action-buttons { gap: 10px; } .action-buttons .btn { padding: 12px 8px; font-size: 14px; min-height: 55px; } .action-buttons .btn .icon { font-size: 22px; } .summary-grid { grid-template-columns: 1fr; } .lock-banner { flex-direction: column; text-align: center; } .lock-banner .lock-text { font-size: 14px; } .user-lock-banner { flex-direction: column; text-align: center; } .user-lock-banner .lock-text { font-size: 13px; } }
        @media (max-width: 600px) { .header-right .nav-btn { font-size: 11px; padding: 4px 10px; } .header-right .user-name-btn { font-size: 11px; padding: 4px 10px; } .header-right .logout-link { font-size: 11px; padding: 4px 10px; } .action-buttons { gap: 8px; } .action-buttons .btn { padding: 10px 6px; font-size: 13px; min-height: 50px; } .action-buttons .btn .icon { font-size: 20px; } .action-buttons .btn .sub-text { font-size: 10px; } }
        @media (max-width: 400px) { .stats { grid-template-columns: 1fr 1fr; gap: 8px; } .stat-card .number { font-size: 20px; } .action-buttons { gap: 6px; } .action-buttons .btn { padding: 8px 5px; font-size: 12px; min-height: 45px; } .action-buttons .btn .icon { font-size: 18px; } .action-buttons .btn .sub-text { font-size: 9px; } }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <div class="header-left"><h2>🏢 ប្រព័ន្ធគ្រប់គ្រងបុគ្គលិក</h2></div>
            <div class="header-right">
                ...
                <button class="nav-btn" onclick="openRequestsModal()" style="background:rgba(255,193,7,0.2);border-color:#ffc107;">
                    📬 សំណើ {% if pending_count > 0 %}<span style="background:red;color:white;border-radius:50%;padding:0 6px;font-size:11px;margin-left:3px;">{{ pending_count }}</span>{% endif %}
                </button>
                <button class="nav-btn" onclick="openSystemLockModal()" style="background:rgba(33,150,243,0.2);border-color:#2196f3;">🔒 បិទ/បើកប្រព័ន្ធ</button>
                {% endif %}
        ...
        <div id="userLockBanner" class="user-lock-banner {% if user_lock.is_locked == 1 %}show{% endif %}">
            <div style="display:flex;align-items:center;gap:8px;">
                ...
                <span class="lock-text" id="userLockText">{% if user_lock.is_locked == 1 %}អ្នកត្រូវបានបិទការចូលធ្វើការដោយ Admin{% if user_lock.auto_unlock_time %} (បើកដោយស្វ័យប្រវត្តិនៅម៉ោង {{ user_lock.auto_unlock_time }}){% endif %}{% endif %}</span>
            </div>
        </div>
        ...
        <div class="stats">
            <div class="stat-card"><span class="stat-icon">👥</span><div class="number">{{ stats.total_users }}</div><div class="label">បុគ្គលិកសរុប</div></div>
            <div class="stat-card"><span class="stat-icon">✅</span><div class="number green">{{ stats.present_today }}</div><div class="label">កំពុងធ្វើការ</div></div>
            <div class="stat-card"><span class="stat-icon">📋</span><div class="number orange">{{ stats.leave_today }}</div><div class="label">សុំច្បាប់ (Approved)</div></div>
            <div class="stat-card"><span class="stat-icon">🚗</span><div class="number purple">{{ stats.mission_today }}</div><div class="label">ចុះបេសកម្ម (Approved)</div></div>
        </div>

        <div class="action-buttons" id="actionButtons">
            <button class="btn btn-checkin" onclick="openCheckInModal()" id="checkinBtn"><span class="icon">✅</span>ចូលធ្វើការ<span class="sub-text">ជ្រើសរើសវគ្គ</span></button>
            <button class="btn btn-checkout" onclick="handleCheckOut()" id="checkoutBtn"><span class="icon">🚪</span>ចេញពីធ្វើការ<span class="sub-text">ចុចដើម្បីចេញ</span></button>
            <button class="btn btn-leave" onclick="handleLeave()"><span class="icon">📋</span>សុំច្បាប់<span class="sub-text">ស្នើសុំច្បាប់</span></button>
            <button class="btn btn-mission" onclick="handleMission()"><span class="icon">🚗</span>បេសកម្ម<span class="sub-text">ស្នើសុំបេសកម្ម</span></button>
        </div>

        <div class="table-container">
            <div class="table-title">📋 ប្រវត្តិការងារបុគ្គលិក <span class="badge-count">{{ work_history|length }}</span></div>
            <table>
                <thead><tr><th>ល.រ</th><th>ឈ្មោះ</th><th>កាលបរិច្ឆេទ</th><th>វគ្គ</th><th>ម៉ោងចូល</th><th>ម៉ោងចេញ</th><th>ម៉ោង/ថ្ងៃ</th><th>ស្ថានភាព</th><th>ព័ត៌មាន</th>{% if session.role == 'admin' %}<th>សកម្មភាព</th>{% endif %}</tr></thead>
                <tbody>
                {% if work_history %}
                    ...
                    <tr>
                        ...
                        <td>{{ item.date }}</td>
                        <td>{% if item.type == 'attendance' %}{% if item.shift == 'វគ្គ 1' %}<span class="shift-badge shift-1">វគ្គ 1</span>{% elif item.shift == 'វគ្គ 2' %}<span class="shift-badge shift-2">វគ្គ 2</span>{% elif item.shift == 'វគ្គ 3' %}<span class="shift-badge shift-3">វគ្គ 3</span>{% else %}<span class="shift-badge shift-1">{{ item.shift }}</span>{% endif %}{% elif item.type == 'leave' %}<span class="shift-badge shift-leave">📋 ច្បាប់</span>{% elif item.type == 'mission' %}<span class="shift-badge shift-mission">🚗 បេសកម្ម</span>{% endif %}</td>
                        <td>{{ item.check_in_time or '' }}</td>
                        <td>{{ item.check_out_time or '' }}</td>
                        <td>{{ item.display_value or '' }}</td>
                        <td>{% if item.type == 'attendance' %}{% if item.status == 'បានបិទ' %}<span class="status-badge present">បានបិទ</span>{% elif item.status == 'កំពុងធ្វើការ' %}<span class="status-badge working">កំពុងធ្វើការ</span>{% else %}<span class="status-badge outside">មិនទាន់ចូល</span>{% endif %}{% else %}<span class="status-badge approved">បានអនុម័ត</span>{% endif %}</td>
                        <td>{{ item.display_info or '' }}</td>
                        {% if session.role == 'admin' %}
                        <td>{% if item.type == 'attendance' %}<a href="/edit_attendance/{{ item.id }}" style="background:#1a73e8;color:white;padding:4px 12px;border-radius:6px;text-decoration:none;font-size:12px;display:inline-block;margin:2px;">✏️ កែ</a><button onclick="deleteAttendance({{ item.id }})" style="background:#dc3545;color:white;padding:4px 12px;border:none;border-radius:6px;cursor:pointer;font-size:12px;margin:2px;">🗑️ លុប</button>{% else %}<span style="font-size:11px;color:#999;">មិនអាចកែ</span>{% endif %}</td>
                        {% endif %}
                    </tr>
                    {% endfor %}
                {% else %}
                    <tr><td colspan="{% if session.role == 'admin' %}10{% else %}9{% endif %}" style="text-align:center;padding:30px;color:#aaa;">📭 មិនមានទិន្នន័យសម្រាប់ខែនេះ</td></tr>
                {% endif %}
                </tbody>
            </table>
        </div>
        ...
        <div class="summary-grid">
            <div class="summary-card">
                ...
                <div class="summary-item" style="font-weight:600;color:#555;border-bottom:2px solid #e8ecf1;"><span>ឈ្មោះ</span><span>ថ្ងៃធ្វើការ</span></div>
                {% for emp in monthly_summary %}<div class="summary-item"><span class="name">{{ emp.full_name }}</span><span class="value">{{ emp.days_worked_display }}</span></div>{% endfor %}
                {% if not monthly_summary %}<div style="text-align:center;padding:15px;color:#aaa;">មិនមានទិន្នន័យ</div>{% endif %}
            </div>
            <div class="summary-card">
                <div class="card-title">📋 សង្ខេបវត្តមាន និងច្បាប់</div>
                <div class="summary-item" style="font-weight:600;color:#555;border-bottom:2px solid #e8ecf1;"><span>ឈ្មោះ</span><span style="display:flex;gap:15px;"><span>ម៉ោង</span><span>ច្បាប់</span><span>បេសកម្ម</span></span></div>
                {% for emp in monthly_summary %}<div class="summary-item"><span class="name">{{ emp.full_name }}</span><span style="display:flex;gap:50px;font-weight:600;"><span class="value hours">{{ emp.total_hours_formatted }}</span><span class="value leave">{{ emp.total_leave_days }}</span><span class="value mission">{{ emp.total_mission_days }}</span></span></div>{% endfor %}
                {% if not monthly_summary %}<div style="text-align:center;padding:15px;color:#aaa;">មិនមានទិន្នន័យ</div>{% endif %}
            </div>
        </div>
    ...
    <div id="checkInModal" class="modal">
        <div class="modal-content">
            ...
            <div id="checkInLocationInfo" class="location-info">⏳ កំពុងចាប់យកទីតាំង...</div>
            <select id="shiftSelect"><option value="1">វគ្គ 1 (ព្រឹក 07:00-11:00)</option><option value="2">វគ្គ 2 (រសៀល 13:00-17:00)</option><option value="3">វគ្គ 3 (យប់ 18:00-08:00)</option></select>
            <div class="btn-group"><button class="btn-success" onclick="submitCheckIn()">✅ ចូលធ្វើការ</button><button class="btn-cancel" onclick="closeModal('checkInModal')">បោះបង់</button></div>
        </div>
    </div>
    ...
    <div id="locationModal" class="modal">
        <div class="modal-content">
            ...
            <div id="locationInfo" class="location-info">{% if company_lat and company_lng %}📍 ទីតាំងបច្ចុប្បន្ន: {{ company_lat }}, {{ company_lng }}{% else %}⚠️ មិនទាន់មានកំណត់ទីតាំង{% endif %}</div>
            ...
            <div class="btn-group"><button class="btn-save" onclick="saveCompanyLocation()">💾 រក្សាទុក</button><button class="btn-cancel" onclick="closeLocationModal()">បោះបង់</button></div>
        </div>
    </div>
    ...
    <div id="systemLockModal" class="modal">
        <div class="modal-content">
            ...
            <div id="lockStatusInfo" class="location-info"><span id="lockStatusText">ប្រព័ន្ធកំពុងបើក</span></div>
            <div style="margin-bottom:15px;"><label style="font-weight:600;color:#555;font-size:14px;">ម៉ោងបើកដោយស្វ័យប្រវត្តិ (HH:MM)</label><input type="time" id="autoUnlockTime" value="06:00" step="60"><div style="font-size:12px;color:#888;margin-top:4px;">🔓 ពេលដល់ម៉ោងនេះ ប្រព័ន្ធនឹងបើកដោយស្វ័យប្រវត្តិ</div></div>
            <div class="btn-group"><button id="lockToggleBtnModal" class="btn-danger" onclick="toggleSystemLockFromModal(1)">🔒 បិទប្រព័ន្ធឥឡូវនេះ</button><button class="btn-success" onclick="toggleSystemLockFromModal(0)" id="unlockBtnModal" style="display:none;">🔓 បើកប្រព័ន្ធឥឡូវនេះ</button><button class="btn-cancel" onclick="closeModal('systemLockModal')">បោះបង់</button></div>
            <div id="unlockScheduledInfo" style="display:none;margin-top:12px;padding:10px;background:#fff3cd;border-radius:8px;color:#856404;font-size:14px;">⏰ ប្រព័ន្ធនឹងបើកដោយស្វ័យប្រវត្តិនៅម៉ោង <span id="scheduledUnlockTime">06:00</span></div>
        </div>
    </div>

    <div id="requestsModal" class="modal">
        <div class="modal-content"><h3>📬 សំណើរង់ចាំការអនុម័ត</h3><div id="requestsList"><p style="text-align:center;color:#888;padding:20px;">កំពុងផ្ទុក...</p></div><div class="btn-group"><button class="btn-cancel" onclick="closeRequestsModal()" style="width:100%;">បិទ</button></div></div>
    </div>
    ...
    window.addEventListener('beforeinstallprompt', function(e) { e.preventDefault(); deferredPrompt = e; var installBtn = document.getElementById('installBtn'); if (installBtn) { installBtn.style.display = 'inline-block'; } });
    document.getElementById('installBtn').addEventListener('click', function() { if (deferredPrompt) { deferredPrompt.prompt(); deferredPrompt.userChoice.then(function(choiceResult) { if (choiceResult.outcome === 'accepted') { document.getElementById('installBtn').style.display = 'none'; } deferredPrompt = null; }); } });
    ...
    if ('serviceWorker' in navigator) { navigator.serviceWorker.register('/static/sw.js').then(function(reg) { console.log('Service Worker registered successfully!'); }).catch(function(err) { console.log('Service Worker registration failed:', err); }); }
    ...
    function updateUserLockUI(locked, autoUnlockTime) {
        ...
        if (locked) {
            ...
            if (autoUnlockTime) { msg += ' (បើកដោយស្វ័យប្រវត្តិនៅម៉ោង ' + autoUnlockTime + ')'; }
    ...
    function updateLockUI(lockStatus) {
        ...
        if (systemLocked) {
            ...
            if (toggleBtn) { toggleBtn.textContent = '🔓 បើកប្រព័ន្ធ'; toggleBtn.style.background = 'rgba(255,255,255,0.2)'; }
            if (lockToggleModal) { lockToggleModal.style.display = 'none'; unlockModal.style.display = 'block'; unlockModal.textContent = '🔓 បើកប្រព័ន្ធជាមួយម៉ោងកំណត់'; }
            if (unlockScheduledInfo) { unlockScheduledInfo.style.display = 'block'; scheduledUnlockTime.textContent = autoUnlockTime; }
            document.getElementById('checkinBtn').classList.add('disabled');
            document.getElementById('checkoutBtn').classList.add('disabled');
            ...
            if (toggleBtn) { toggleBtn.textContent = '🔒 បិទប្រព័ន្ធ'; toggleBtn.style.background = 'rgba(220,53,69,0.3)'; }
            if (lockToggleModal) { lockToggleModal.style.display = 'block'; lockToggleModal.textContent = '🔒 បិទប្រព័ន្ធឥឡូវនេះ'; unlockModal.style.display = 'none'; }
            if (unlockScheduledInfo) { unlockScheduledInfo.style.display = 'none'; }
            if (!userLocked) { document.getElementById('checkinBtn').classList.remove('disabled'); document.getElementById('checkoutBtn').classList.remove('disabled'); }
            var statusMsg = document.getElementById('statusMessage');
            if (statusMsg.className === 'status-message locked') { statusMsg.className = 'status-message info'; statusMsg.innerHTML = '🔵 ប្រព័ន្ធកំពុងបើក។ អ្នកអាចចុចចូល/ចេញធ្វើការបាន!'; statusMsg.style.display = 'block'; }
        }
        var statusText = document.getElementById('lockStatusText');
        if (statusText) {
            if (systemLocked) { statusText.textContent = '🔒 ប្រព័ន្ធកំពុងបិទ (បើកដោយស្វ័យប្រវត្តិនៅម៉ោង ' + autoUnlockTime + ')'; statusText.style.color = '#dc3545'; }
            else { statusText.textContent = '🔓 ប្រព័ន្ធកំពុងបើក'; statusText.style.color = '#34a853'; }
        }
    }

    function getSystemLockStatus() { fetch('/get_system_lock_status').then(function(res) { return res.json(); }).then(function(data) { updateLockUI(data); }).catch(function(err) { console.log('Lock status error:', err); }); }

    function openSystemLockModal() { getSystemLockStatus(); var autoTime = document.getElementById('autoUnlockTime'); if (autoTime) { autoTime.value = autoUnlockTime || '06:00'; } openModal('systemLockModal'); }
    ...
    function toggleSystemLockFromModal(lockState) {
        ...
        if (!unlockTime) { unlockTime = '06:00'; }
        var message = '';
        if (lockState === 1) { message = 'តើអ្នកចង់បិទប្រព័ន្ធចូលធ្វើការមែនទេ?'; }
        else { message = 'តើអ្នកចង់កំណត់ម៉ោងបើកប្រព័ន្ធដោយស្វ័យប្រវត្តិនៅម៉ោង ' + unlockTime + ' មែនទេ?\n\nប្រព័ន្ធនឹងបើកដោយស្វ័យប្រវត្តិនៅម៉ោង ' + unlockTime; }
        if (!confirm(message)) { return; }
        var actualLockState = (lockState === 1) ? 1 : 1;
        var isUnlockRequest = (lockState === 0);
        var newState = isUnlockRequest ? 1 : lockState;
        fetch('/toggle_system_lock', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ lock_state: newState, auto_unlock_time: unlockTime })
        }).then(function(res) { return res.json(); }).then(function(result) {
            alert(result.message);
            if (result.success) {
                if (isUnlockRequest) { alert('⏰ ប្រព័ន្ធនឹងបើកដោយស្វ័យប្រវត្តិនៅម៉ោង ' + unlockTime); }
                getSystemLockStatus();
                if (document.getElementById('systemLockModal').classList.contains('show')) { closeModal('systemLockModal'); }
            }
        }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
    }

    function checkDataVersion() { fetch('/get_data_version').then(function(res) { return res.json(); }).then(function(data) { if (data.version > currentDataVersion) { console.log('Data changed! Refreshing...'); currentDataVersion = data.version; location.reload(); } }).catch(function(err) { console.log('Data version check error:', err); }); }
    function startDataVersionChecker() { checkDataVersion(); dataCheckInterval = setInterval(checkDataVersion, 3000); }
    function stopDataVersionChecker() { if (dataCheckInterval) { clearInterval(dataCheckInterval); dataCheckInterval = null; } }

    function updateButtonStatus() {
        fetch('/get_checkin_status').then(function(res) { return res.json(); }).then(function(data) {
            var checkinBtn = document.getElementById('checkinBtn');
            var checkoutBtn = document.getElementById('checkoutBtn');
            if (systemLocked || userLocked) { checkinBtn.classList.add('disabled'); checkoutBtn.classList.add('disabled'); return; }
            checkinBtn.disabled = false; checkoutBtn.disabled = false;
            if (data.has_checkin) {
                var shiftNames = {1: 'វគ្គ 1', 2: 'វគ្គ 2', 3: 'វគ្គ 3'};
                var shiftText = shiftNames[data.shift] || '';
                var statusMsg = document.getElementById('statusMessage');
                statusMsg.className = 'status-message warning';
                statusMsg.innerHTML = '🟢 អ្នកកំពុងធ្វើការ (' + shiftText + ') ចាប់ពីម៉ោង ' + (data.check_in_time ? data.check_in_time.split(' ')[1].slice(0,5) : '') + '។ សូមចុច "ចេញពីធ្វើការ" មុនពេលចូលម្តងទៀត!';
                statusMsg.style.display = 'block';
            } else if (!systemLocked && !userLocked) {
                var statusMsg = document.getElementById('statusMessage');
                statusMsg.className = 'status-message info';
                statusMsg.innerHTML = '🔵 អ្នកមិនទាន់ចូលធ្វើការនៅថ្ងៃនេះទេ។ សូមចុច "ចូលធ្វើការ" ដើម្បីចាប់ផ្តើម!';
                statusMsg.style.display = 'block';
            }
    ...
    function cleanData(type) {
        var typeNames = { 'all': 'ទិន្នន័យទាំងអស់', 'attendance': 'ទិន្នន័យវត្តមាន', 'leaves': 'ទិន្នន័យសុំច្បាប់', 'missions': 'ទិន្នន័យបេសកម្ម' };
        ...
        fetch('/clean_data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: type }) }).then(function(response) { return response.json(); }).then(function(result) { alert(result.message); if (result.success) { closeModal('cleanModal'); location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err); });
    }

    function loadRequests() {
        fetch('/get_pending_requests').then(function(res) { return res.json(); }).then(function(data) {
            var html = '';
            if (data.leaves.length === 0 && data.missions.length === 0) { html = '<p style="text-align:center;color:#888;padding:20px;">📭 មិនមានសំណើរង់ចាំ</p>'; }
            for (var i = 0; i < data.leaves.length; i++) {
                var item = data.leaves[i];
                var attachmentHtml = '';
                if (item.attachment) {
                    var ext = item.attachment.split('.').pop().toLowerCase();
                    if (['jpg','jpeg','png','gif','bmp','webp'].indexOf(ext) !== -1) { attachmentHtml = '<div style="margin-top:5px;"><a href="' + item.attachment + '" target="_blank" style="color:#1a73e8;text-decoration:none;">🖼️ មើលរូបភាព</a></div>'; }
                    else { attachmentHtml = '<div style="margin-top:5px;"><a href="' + item.attachment + '" target="_blank" style="color:#1a73e8;text-decoration:none;">📎 ទាញយកឯកសារ</a></div>'; }
                }
                html += '<div class="request-item"><div class="request-user">📋 ' + item.full_name + ' (' + item.username + ')</div><div class="request-detail">សុំច្បាប់: ' + item.days + ' ថ្ងៃ | ' + item.start_date + ' ដល់ ' + item.end_date + ' | មូលហេតុ: ' + (item.reason || 'មិនបានបញ្ជាក់') + '</div>' + attachmentHtml + '<div class="request-actions"><button class="approve-btn" onclick="approveRequest(\'leave\', ' + item.id + ')">✅ អនុម័ត</button><button class="reject-btn" onclick="rejectRequest(\'leave\', ' + item.id + ')">❌ បដិសេធ</button></div></div>';
            }
            for (var j = 0; j < data.missions.length; j++) {
                ...
                if (item2.attachment) {
                    ...
                    if (['jpg','jpeg','png','gif','bmp','webp'].indexOf(ext2) !== -1) { attachmentHtml2 = '<div style="margin-top:5px;"><a href="' + item2.attachment + '" target="_blank" style="color:#1a73e8;text-decoration:none;">🖼️ មើលរូបភាព</a></div>'; }
                    else { attachmentHtml2 = '<div style="margin-top:5px;"><a href="' + item2.attachment + '" target="_blank" style="color:#1a73e8;text-decoration:none;">📎 ទាញយកឯកសារ</a></div>'; }
                }
                html += '<div class="request-item"><div class="request-user">🚗 ' + item2.full_name + ' (' + item2.username + ')</div><div class="request-detail">បេសកម្ម: ' + item2.days + ' ថ្ងៃ | ' + item2.start_date + ' ដល់ ' + item2.end_date + ' | ទីតាំង: ' + (item2.destination || 'មិនបានបញ្ជាក់') + '</div>' + attachmentHtml2 + '<div class="request-actions"><button class="approve-btn" onclick="approveRequest(\'mission\', ' + item2.id + ')">✅ អនុម័ត</button><button class="reject-btn" onclick="rejectRequest(\'mission\', ' + item2.id + ')">❌ បដិសេធ</button></div></div>';
            }
            document.getElementById('requestsList').innerHTML = html;
        }).catch(function(err) { document.getElementById('requestsList').innerHTML = '<p style="text-align:center;color:red;">មានបញ្ហា: ' + err + '</p>'; });
    }

    function approveRequest(type, id) {
        if (!confirm('តើអ្នកចង់អនុម័តសំណើនេះមែនទេ?')) return;
        fetch('/approve_request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: type, id: id }) }).then(function(res) { return res.json(); }).then(function(result) { alert(result.message); if (result.success) { loadRequests(); window.location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
    }

    function rejectRequest(type, id) {
        if (!confirm('តើអ្នកចង់បដិសេធសំណើនេះមែនទេ?')) return;
        fetch('/reject_request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: type, id: id }) }).then(function(res) { return res.json(); }).then(function(result) { alert(result.message); if (result.success) { loadRequests(); window.location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
    }

    function getCurrentLocation() {
        if (!navigator.geolocation) { alert('កម្មវិធីរុករករបស់អ្នកមិនគាំទ្រការចាប់ទីតាំងទេ!'); return; }
        navigator.geolocation.getCurrentPosition(function(pos) { document.getElementById('companyLat').value = pos.coords.latitude; document.getElementById('companyLng').value = pos.coords.longitude; document.getElementById('locationInfo').innerHTML = '📍 ទីតាំងបច្ចុប្បន្ន: ' + pos.coords.latitude + ', ' + pos.coords.longitude; }, function(err) { alert('មិនអាចចាប់យកទីតាំងបាន! សូមបញ្ចូលដោយដៃ។'); console.log(err); });
    }
    ...
    function saveCompanyLocation() {
        ...
        if (!lat || !lng) { alert('សូមបញ្ចូលទីតាំងឲ្យបានពេញលេញ!'); return; }
        fetch('/save_location', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lat: parseFloat(lat), lng: parseFloat(lng) }) }).then(function(res) { return res.json(); }).then(function(data) { if (data.success) { alert('✅ រក្សាទុកទីតាំងជោគជ័យ!'); document.getElementById('locationInfo').innerHTML = '📍 ទីតាំងបច្ចុប្បន្ន: ' + lat + ', ' + lng; closeLocationModal(); location.reload(); } else { alert('❌ ' + data.message); } }).catch(function(err) { alert('មានបញ្ហា: ' + err); });
    }

    function openCheckInModal() {
        if (systemLocked) { alert('⛔ ប្រព័ន្ធកំពុងបិទការចូលធ្វើការ! សូមរង់ចាំរហូតដល់ម៉ោង ' + autoUnlockTime + ' ឬទាក់ទង Admin!'); return; }
        if (userLocked) { alert('⛔ អ្នកត្រូវបានបិទការចូលធ្វើការដោយ Admin! សូមទាក់ទង Admin!'); return; }
        if (!navigator.geolocation) { alert('កម្មវិធីរុករករបស់អ្នកមិនគាំទ្រការចាប់ទីតាំងទេ!'); return; }
        document.getElementById('checkInLocationInfo').innerHTML = '⏳ កំពុងចាប់យកទីតាំង...';
        navigator.geolocation.getCurrentPosition(function(pos) { userLocation = { lat: pos.coords.latitude, lng: pos.coords.longitude }; document.getElementById('checkInLocationInfo').innerHTML = '📍 ទីតាំងរបស់អ្នក: ' + pos.coords.latitude.toFixed(6) + ', ' + pos.coords.longitude.toFixed(6) + '<br>📏 កំពុងពិនិត្យចម្ងាយ...'; openModal('checkInModal'); }, function(err) { document.getElementById('checkInLocationInfo').innerHTML = '⚠️ មិនអាចចាប់យកទីតាំង! សូមបើក GPS ហើយចុចសាកល្បងម្តងទៀត<br><button onclick="openCheckInModal()" style="margin-top:10px;padding:8px 20px;background:#1a73e8;color:white;border:none;border-radius:8px;cursor:pointer;">🔄 សាកល្បងម្តងទៀត</button>'; console.log(err); });
    }

    function submitCheckIn() {
        if (!userLocation) { alert('សូមចាប់យកទីតាំងរបស់អ្នកមុន!'); return; }
        var shift = document.getElementById('shiftSelect').value;
        fetch('/get_company_location').then(function(res) { return res.json(); }).then(function(data) {
            if (!data.lat || !data.lng) { alert('សូមឲ្យ Admin កំណត់ទីតាំងក្រុមហ៊ុនជាមុនសិន!'); return; }
            fetch('/check_in', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_lat: userLocation.lat, user_lng: userLocation.lng, company_lat: data.lat, company_lng: data.lng, shift: parseInt(shift) }) }).then(function(res) { return res.json(); }).then(function(result) { alert(result.message); if (result.success) { closeModal('checkInModal'); window.location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
        }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
    }
    ...
    function handleCheckOut() {
        ...
        if (!navigator.geolocation) { alert('កម្មវិធីរុករករបស់អ្នកមិនគាំទ្រការចាប់ទីតាំងទេ!'); return; }
        var statusMsg = document.getElementById('statusMessage');
        if (statusMsg) { statusMsg.className = 'status-message info'; statusMsg.innerHTML = '⏳ កំពុងចាប់យកទីតាំងសម្រាប់ចេញធ្វើការ...'; statusMsg.style.display = 'block'; }
        navigator.geolocation.getCurrentPosition(function(pos) { var location = { lat: pos.coords.latitude, lng: pos.coords.longitude }; fetch('/get_company_location').then(function(res) { return res.json(); }).then(function(data) { if (!data.lat || !data.lng) { alert('សូមឲ្យ Admin កំណត់ទីតាំងក្រុមហ៊ុនជាមុនសិន!'); return; } fetch('/check_out', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_lat: location.lat, user_lng: location.lng, company_lat: data.lat, company_lng: data.lng }) }).then(function(res) { return res.json(); }).then(function(result) { alert(result.message); if (result.success) { window.location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); }); }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); }); }, function(err) { var statusMsg = document.getElementById('statusMessage'); if (statusMsg) { statusMsg.className = 'status-message danger'; statusMsg.innerHTML = '⚠️ មិនអាចចាប់យកទីតាំង! សូមបើក GPS ហើយព្យាយាមម្តងទៀត'; statusMsg.style.display = 'block'; } console.log('Geolocation error:', err); });
    }

    function handleLeave() {
        var html = '<div style="padding:10px;"><div style="margin-bottom:10px;"><label>ចំនួនថ្ងៃ (អាចជា 0.5):</label><input type="number" id="leaveDays" step="0.5" min="0.5" value="1" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>ចាប់ពីថ្ងៃ:</label><input type="date" id="leaveStartDate" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>ដល់ថ្ងៃ:</label><input type="date" id="leaveEndDate" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>មូលហេតុ:</label><textarea id="leaveReason" rows="3" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;font-family:\'Khmer OS\',Arial;" placeholder="សូមបញ្ចូលមូលហេតុ..."></textarea></div><div style="margin-bottom:10px;"><label>ឯកសារភ្ជាប់ (រូបភាព ឬ PDF):</label><input type="file" id="leaveAttachment" accept="image/*,.pdf,.doc,.docx" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;font-family:\'Khmer OS\',Arial;"><div style="font-size:12px;color:#888;margin-top:4px;">📎 អាចភ្ជាប់រូបភាព, PDF, Word (អតិបរមា 5MB)</div></div><div id="attachmentPreview" style="display:none;margin-bottom:10px;padding:10px;background:#f8f9fa;border-radius:8px;text-align:center;"><img id="previewImage" src="" style="max-width:200px;max-height:200px;border-radius:8px;display:none;"><a id="previewLink" href="#" target="_blank" style="display:none;color:#1a73e8;text-decoration:none;">📄 មើលឯកសារ</a><button onclick="removeAttachment()" style="display:block;margin-top:5px;padding:4px 12px;background:#dc3545;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">❌ លុប</button></div><div style="display:flex;gap:10px;margin-top:10px;"><button onclick="submitLeave()" style="flex:1;padding:10px;background:#34a853;color:white;border:none;border-radius:8px;cursor:pointer;font-size:16px;font-family:\'Khmer OS\',Arial;">✅ បញ្ជូន</button><button onclick="closeModal(\'leaveModal\')" style="flex:1;padding:10px;background:#e8ecf1;color:#333;border:none;border-radius:8px;cursor:pointer;font-size:16px;font-family:\'Khmer OS\',Arial;">បោះបង់</button></div></div>';
        var modal = document.createElement('div');
        modal.id = 'leaveModal';
        modal.className = 'modal show';
        modal.innerHTML = '<div class="modal-content" style="max-width:500px;"><h3>📋 សុំច្បាប់</h3>' + html + '</div>';
        ...
        fileInput.addEventListener('change', function(e) { var file = this.files[0]; if (file) { var previewDiv = document.getElementById('attachmentPreview'); var previewImage = document.getElementById('previewImage'); var previewLink = document.getElementById('previewLink'); previewDiv.style.display = 'block'; previewImage.style.display = 'none'; previewLink.style.display = 'none'; if (file.type.startsWith('image/')) { var reader = new FileReader(); reader.onload = function(e) { previewImage.src = e.target.result; previewImage.style.display = 'block'; }; reader.readAsDataURL(file); } else { previewLink.textContent = '📄 ' + file.name; previewLink.style.display = 'block'; previewLink.href = '#'; previewLink.onclick = function(e) { e.preventDefault(); alert('ឯកសារ: ' + file.name + '\nទំហំ: ' + (file.size / 1024).toFixed(2) + ' KB'); }; } } });
    }

    function removeAttachment() { var fileInput = document.getElementById('leaveAttachment'); fileInput.value = ''; document.getElementById('attachmentPreview').style.display = 'none'; document.getElementById('previewImage').style.display = 'none'; document.getElementById('previewLink').style.display = 'none'; }
    ...
    function submitLeave() {
        ...
        if (!start_date || !end_date) { alert('សូមជ្រើសរើសថ្ងៃ!'); return; }
        if (!reason) { alert('សូមបញ្ចូលមូលហេតុ!'); return; }
        if (parseFloat(days) <= 0) { alert('សូមបញ្ចូលចំនួនថ្ងៃឲ្យបានត្រឹមត្រូវ!'); return; }
        if (file && file.size > 5 * 1024 * 1024) { alert('ឯកសារធំពេក! សូមជ្រើសរើសឯកសារដែលមានទំហំតិចជាង 5MB'); return; }
        ...
        fetch('/request_leave', { method: 'POST', body: formData }).then(function(response) { return response.json(); }).then(function(result) { alert(result.message); if (result.success) { closeModal('leaveModal'); window.location.reload(); } }).catch(function(err) { console.error("Error:", err); alert('មានបញ្ហា: ' + err.message); });
    }

    function handleMission() {
        var html = '<div style="padding:10px;"><div style="margin-bottom:10px;"><label>ចំនួនថ្ងៃ (អាចជា 0.5):</label><input type="number" id="missionDays" step="0.5" min="0.5" value="1" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>ចាប់ពីថ្ងៃ:</label><input type="date" id="missionStartDate" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>ដល់ថ្ងៃ:</label><input type="date" id="missionEndDate" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;"></div><div style="margin-bottom:10px;"><label>ទីតាំងបេសកម្ម:</label><input type="text" id="missionDestination" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;font-family:\'Khmer OS\',Arial;" placeholder="សូមបញ្ចូលទីតាំង..."></div><div style="margin-bottom:10px;"><label>គោលបំណង:</label><textarea id="missionPurpose" rows="3" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;font-family:\'Khmer OS\',Arial;" placeholder="សូមបញ្ចូលគោលបំណង..."></textarea></div><div style="margin-bottom:10px;"><label>ឯកសារភ្ជាប់ (រូបភាព ឬ PDF):</label><input type="file" id="missionAttachment" accept="image/*,.pdf,.doc,.docx" style="width:100%;padding:8px;border:2px solid #e8ecf1;border-radius:8px;font-family:\'Khmer OS\',Arial;"><div style="font-size:12px;color:#888;margin-top:4px;">📎 អាចភ្ជាប់រូបភាព, PDF, Word (អតិបរមា 5MB)</div></div><div id="missionAttachmentPreview" style="display:none;margin-bottom:10px;padding:10px;background:#f8f9fa;border-radius:8px;text-align:center;"><img id="missionPreviewImage" src="" style="max-width:200px;max-height:200px;border-radius:8px;display:none;"><a id="missionPreviewLink" href="#" target="_blank" style="display:none;color:#1a73e8;text-decoration:none;">📄 មើលឯកសារ</a><button onclick="removeMissionAttachment()" style="display:block;margin-top:5px;padding:4px 12px;background:#dc3545;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">❌ លុប</button></div><div style="display:flex;gap:10px;margin-top:10px;"><button onclick="submitMission()" style="flex:1;padding:10px;background:#1a73e8;color:white;border:none;border-radius:8px;cursor:pointer;font-size:16px;font-family:\'Khmer OS\',Arial;">✅ បញ្ជូន</button><button onclick="closeModal(\'missionModal\')" style="flex:1;padding:10px;background:#e8ecf1;color:#333;border:none;border-radius:8px;cursor:pointer;font-size:16px;font-family:\'Khmer OS\',Arial;">បោះបង់</button></div></div>';
        var modal = document.createElement('div');
        modal.id = 'missionModal';
        modal.className = 'modal show';
        modal.innerHTML = '<div class="modal-content" style="max-width:500px;"><h3>🚗 សុំបេសកម្ម</h3>' + html + '</div>';
        document.body.appendChild(modal);
        var today = new Date().toISOString().split('T')[0];
        ...
        fileInput.addEventListener('change', function(e) { var file = this.files[0]; if (file) { var previewDiv = document.getElementById('missionAttachmentPreview'); var previewImage = document.getElementById('missionPreviewImage'); var previewLink = document.getElementById('missionPreviewLink'); previewDiv.style.display = 'block'; previewImage.style.display = 'none'; previewLink.style.display = 'none'; if (file.type.startsWith('image/')) { var reader = new FileReader(); reader.onload = function(e) { previewImage.src = e.target.result; previewImage.style.display = 'block'; }; reader.readAsDataURL(file); } else { previewLink.textContent = '📄 ' + file.name; previewLink.style.display = 'block'; previewLink.href = '#'; previewLink.onclick = function(e) { e.preventDefault(); alert('ឯកសារ: ' + file.name + '\nទំហំ: ' + (file.size / 1024).toFixed(2) + ' KB'); }; } } });
    }

    function removeMissionAttachment() { var fileInput = document.getElementById('missionAttachment'); fileInput.value = ''; document.getElementById('missionAttachmentPreview').style.display = 'none'; document.getElementById('missionPreviewImage').style.display = 'none'; document.getElementById('missionPreviewLink').style.display = 'none'; }
    ...
    function submitMission() {
        ...
        var file = fileInput.files[0];
        if (!start_date || !end_date) { alert('សូមជ្រើសរើសថ្ងៃ!'); return; }
        if (!destination) { alert('សូមបញ្ចូលទីតាំងបេសកម្ម!'); return; }
        if (parseFloat(days) <= 0) { alert('សូមបញ្ចូលចំនួនថ្ងៃឲ្យបានត្រឹមត្រូវ!'); return; }
        if (file && file.size > 5 * 1024 * 1024) { alert('ឯកសារធំពេក! សូមជ្រើសរើសឯកសារដែលមានទំហំតិចជាង 5MB'); return; }
        var formData = new FormData();
        formData.append('start_date', start_date);
        ...
        if (file) { formData.append('attachment', file); }
        fetch('/request_mission', { method: 'POST', body: formData }).then(function(response) { return response.json(); }).then(function(result) { alert(result.message); if (result.success) { closeModal('missionModal'); window.location.reload(); } }).catch(function(err) { console.error("Error:", err); alert('មានបញ្ហា: ' + err.message); });
    }

    function checkNewRequests() {
        fetch('/check_new_requests').then(function(res) { return res.json(); }).then(function(data) {
            var requestBtn = document.querySelector('.header-right .nav-btn[onclick="openRequestsModal()"]');
            if (requestBtn) { var oldBadge = requestBtn.querySelector('span'); if (oldBadge) { oldBadge.remove(); } if (data.count > 0) { var badge = document.createElement('span'); badge.style.cssText = 'background:red;color:white;border-radius:50%;padding:0 6px;font-size:11px;margin-left:3px;'; badge.textContent = data.count; requestBtn.appendChild(badge); } }
            if (!isFirstCheck && data.new && data.message) { showNotification(data.message); }
            isFirstCheck = false;
    ...
    function deleteAttendance(id) {
        ...
        fetch('/delete_attendance/' + id, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then(function(response) { console.log('Response status:', response.status); return response.json(); }).then(function(result) { console.log('Result:', result); alert(result.message); if (result.success) { window.location.reload(); } }).catch(function(err) { console.error('Error:', err); alert('មានបញ្ហា: ' + err.message); });
    }
    ...
    {% if session.role == 'admin' %}startNotificationChecker();{% endif %}
    setInterval(updateButtonStatus, 10000);
    setInterval(getSystemLockStatus, 60000);
    window.addEventListener('beforeunload', function() { stopDataVersionChecker(); stopNotificationChecker(); });
    </script>
</body>
</html>
'''
